...
import type { QuestionBreakdown } from '@/lib/dashboard-types';
...
function QuestionDeltaCard({ question }: { question: QuestionBreakdown }) {
  ...
  const { label: gLabel, colour: gColour } = delta !== null
    ? gapLabel(delta)
    : { label: 'No comparison', colour: 'bg-gray-100 text-gray-500' };
    ...
    <div className="rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-3 flex items-center gap-4 border-b border-gray-100">
        ...
        <div className="flex-1 text-center">
          <p className="text-2xs text-gray-400 mb-0.5">Team Manager Assessment</p>
          <p className="text-2xl font-bold text-orbit-forest">
        ...
        {delta !== null && (
          <div className="flex-shrink-0 text-center">
            <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${gColour}`}>
              Δ {Math.abs(delta).toFixed(1)} — {gLabel}
            </div>
          </div>
        )}
        ...
        {/* Member score */}
        <div className="flex-1 text-center">
          <p className="text-2xs text-gray-400 mb-0.5">Team Member Assessment</p>
          <p className="text-2xl font-bold text-orbit-amber">
      ...
      <div className="grid grid-cols-2 divide-x divide-gray-100">
        ...
        <div className="px-4 py-4">
          ...
          {noMgr ? (
            <p className="text-xs text-gray-400 italic">No manager responses</p>
          ) : (
        ...
        <div className="px-4 py-4">
          ...
          {noMem ? (
            <p className="text-xs text-gray-400 italic">No member responses</p>
          ) : (
            <div className="space-y-2">
      ...
      {question.delta !== null && Math.abs(question.delta) >= 0.5 && (
        <div className="px-5 py-3 bg-blue-50/60 border-t border-blue-100">
          ...
          <p className="text-xs text-blue-800 leading-relaxed">
            {question.managerScore! > question.memberScore!
              ? `The manager rates this ${question.delta.toFixed(1)} points higher than team members. This may indicate a candour gap — the team may be experiencing challenges that aren't yet visible to the manager.`
              : `Team members rate this ${Math.abs(question.delta).toFixed(1)} points higher than the manager. Members may feel more confident in this area than leadership recognises.`}
          </p>
        </div>
      )}
    </div>
...
export default async function DeltaPage() {
  ...
  if (!data || data.questionBreakdowns.length === 0) {
    return (
